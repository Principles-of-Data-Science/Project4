...
Using the `MatchIt` package, fit a propensity score model using all the predictors **except** the `cat2` variable (which has too many `NA` values). The propensity score model should specify a matching ratio of 1:1.
